...
exports.author_create_post = [
	...
	body('date_of_death', 'Inavlid date of birth').optional({checkFalsy: true}).isISO8601(),
...
exports.author_delete_post = function (req, res, next) {
	async.parallel({
		...
		if (results.authors_books.length > 0) {
			// Author has books. Render in the same ways as for GET route
			res.render('author_delete', { title: 'Delete Author', author: results.author, author_books: results.authors_books});
			return;
		} else {
			// Author has no books. Delete object and redirecto the list of authors.
	...
	Author.findById(req.params.id)
		.exec(function (err, author) {
			...
			if (author === null) { // No results
				var err = new Err('Author not found');
				err.status = 404;
				return next(err);
			}
